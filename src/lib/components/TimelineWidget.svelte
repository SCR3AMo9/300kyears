<script lang="ts">
  ...
  let cumulativeEvents = $derived(() => {
    ...
    for (let i = 0; i <= eventEraIdx; i++) {
      ...
      if (isEventEra) {
        const activeEventIndex = era.events.findIndex(
  ...
  function calculateMetric(
    ...
    return Math.min(
      100,
      Math.max(0, baseline + getTagModifier(tagCounts, modifiers))
    );
  }

  // Get current era baseline values (for dot positioning on graph)
  let currentEraBaseline = $derived(
    ERA_BASELINES[activeEraIndex] || { what: 50, why: 50, how: 50 }
  );
  let currentEraConsciousness = $derived(
    Math.min(
      100,
      currentEraBaseline.what * 0.35 +
        currentEraBaseline.why * 0.4 +
        currentEraBaseline.how * 0.25
    )
  );
  ...
  let currentConsciousness = $derived(() => {
    ...
    return Math.min(100, what * 0.35 + why * 0.4 + how * 0.25);
  });
  ...
  let graphDataPoints = $derived(() => {
    ...
    for (let eraIdx = 0; eraIdx < eras.length; eraIdx++) {
      ...
      const consciousness = Math.min(100, what * 0.35 + why * 0.4 + how * 0.25);
  ...
  function getSmoothPath(
    metric: "what" | "why" | "how" | "consciousness"
  ): string {
    const width = 400;
    const height = 100;
  ...
  let revealClipPath = $derived(() => {
    ...
    if (eventEraIdx === -1) {
      // Fallback to era-based reveal (start of current era)
      return `inset(0 ${100 - (activeEraIndex / eras.length) * 100}% 0 0)`;
    }
  ...
  <div class="graph-section">
    <div class="graph-header">
      <span class="section-label">
        <span class="label-what">WHAT</span> ·
        <span class="label-why">WHY</span>
        · <span class="label-how">HOW</span> ·
        <span class="label-consciousness">Consciousness</span>
      </span>
    </div>
    <svg class="graph" viewBox="0 0 400 100" preserveAspectRatio="none">
      ...
      <path
        d={getSmoothPath("how")}
        class="graph-line how-line dimmed"
        fill="none"
      />
      <path
        d={getSmoothPath("what")}
        class="graph-line what-line dimmed"
        fill="none"
      />
      <path
        d={getSmoothPath("why")}
        class="graph-line why-line dimmed"
        fill="none"
      />
      <path
        d={getSmoothPath("consciousness")}
        class="graph-line consciousness-line dimmed"
        fill="none"
      />
      ...
      <g style="clip-path: {revealClipPath()}">
        <path
          d={getSmoothPath("how")}
          class="graph-line how-line"
          fill="none"
        />
        <path
          d={getSmoothPath("what")}
          class="graph-line what-line"
          fill="none"
        />
        <path
          d={getSmoothPath("why")}
          class="graph-line why-line"
          fill="none"
        />
        <path
          d={getSmoothPath("consciousness")}
          class="graph-line consciousness-line"
          fill="none"
        />
      </g>
      <!-- Position marker -->
      <line
        x1={getCurrentMarkerX()}
        y1="0"
        x2={getCurrentMarkerX()}
        y2="100"
...
<style>
  ...
  .tag-box[data-type="hammerfall"].filled {
    background: #2d2d2d;
  }
  .tag-box[data-type="collapse"].filled {
    background: #4a4a4a;
  }
  .tag-box[data-type="stress"].filled {
    background: #a67c52;
  }
  .tag-box[data-type="expansion"].filled {
    background: #b8942a;
  }
  .tag-box[data-type="transition"].filled {
    background: #6a6a6a;
  }
  .tag-box[data-type="recovery"].filled {
    background: #808080;
  }
  .tag-box[data-type="parallel"].filled {
    background: #707070;
  }

  .tag-box[data-type="tech"].filled {
    background: #4a5a68;
  }
  .tag-box[data-type="cognitive"].filled {
    background: #6a5a7a;
  }
  .tag-box[data-type="climate"].filled {
    background: #5a6878;
  }
  .tag-box[data-type="migration"].filled {
    background: #8a7a60;
  }
  .tag-box[data-type="external"].filled {
    background: #3a3235;
  }

  .tag-box[data-type="gain"].filled {
    background: #a08820;
  }
  .tag-box[data-type="lost"].filled {
    background: #3d3d3d;
  }

  /* Tag name pill styles (matching content tags) */
  .tag-name[data-type="hammerfall"] {
    background: #2d2d2d;
    color: #c08080;
  }
  .tag-name[data-type="collapse"] {
    background: #4a4a4a;
    color: #d0d0d0;
  }
  .tag-name[data-type="stress"] {
    background: #f0e2d0;
    color: #a67c52;
  }
  .tag-name[data-type="expansion"] {
    background: #f5ecd4;
    color: #b8942a;
  }
  .tag-name[data-type="transition"] {
    background: #e8e8e8;
    color: #6a6a6a;
  }
  .tag-name[data-type="recovery"] {
    background: #f8f8f8;
    color: #808080;
  }
  .tag-name[data-type="parallel"] {
    background: #e8e8e8;
    color: #707070;
  }

  .tag-name[data-type="tech"] {
    background: #e0e5e8;
    color: #4a5a68;
  }
  .tag-name[data-type="cognitive"] {
    background: #ebe4f0;
    color: #6a5a7a;
  }
  .tag-name[data-type="climate"] {
    background: #e4eaef;
    color: #5a6878;
  }
  .tag-name[data-type="migration"] {
    background: #f2ebe0;
    color: #8a7a60;
  }
  .tag-name[data-type="external"] {
    background: #3a3235;
    color: #c0a8a8;
  }

  .tag-name[data-type="gain"] {
    background: #f5ecc0;
    color: #a08820;
  }
  .tag-name[data-type="lost"] {
    background: #3d3d3d;
    color: #909090;
  }
</style>
