<script lang="ts">
  ...
  function isEventPast(eraIdx: number, eventIdx: number): boolean {
    ...
    const activeEventIndex = currentEra.events.findIndex(
...
<nav class="toc">
  ...
  <div class="toc-content">
    {#each eras as era, eraIdx}
      ...
      >
        <span
          class="toc-marker era-marker"
          class:filled={isEraPast(eraIdx) || isEraActive(eraIdx)}
        ></span>
      ...
      {#each era.events.filter((e) => !containsTechTag(e)) as event, eventIdx}
        ...
        >
          <span
            class="toc-marker event-marker"
            class:filled={isEventPast(eraIdx, eventIdx)}
          ></span>
