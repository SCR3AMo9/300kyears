  ...
  <header class="header">
    <div class="header-content">
      <div class="title-wrapper">
        <img src={logo} alt="300k Years Logo" class="site-logo" />
      ...
      <article class="article">
        <section id="introduction" class="intro-section">
          <p class="intro-lead">
            ...
            300,000 years—from the first burials that suggest consciousness of
            death, through the catastrophic Laschamp Excursion that may have
            triggered symbolic thought, to the present moment where AI threatens
            to reshape how we construct meaning itself.
          </p>

          <p>
            Each era represents a distinct phase in how our species has
            processed existence: the interplay of <strong>WHAT</strong> we
            experience and know to be fact, <strong>WHY</strong> we think it
            matters, and
            <strong>HOW</strong> we respond. We will try to find patterns behind
            the patterns: <em>Expansions</em> when humans dominate,
            <em>Collapses</em>
            when systems fail, and <em>Hammerfalls</em>—those catastrophic
            moments when the sky breaks and everything must be reinvented.
          </p>
        </section>
        ...
        <section id="timeline" class="timeline-section">
          {#each eras as era (era.id)}
            <article
              class="era"
              class:expanded={expandedEras.has(era.id)}
              id={era.id}
            >
              <!-- Collapsed era header (clickable) -->
              <button
                class="era-collapsed-header"
                onclick={() => toggleEra(era.id)}
              >
                <span
                  class="era-toggle"
                  class:expanded={expandedEras.has(era.id)}>&#9658;</span
                >
              ...
              {#if expandedEras.has(era.id)}
                ...
                {#if eraSummaries[era.id]}
                  <div class="era-summary-section">
                    <button
                      class="era-summary-header"
                      onclick={() => toggleEraSummary(era.id)}
                    >
                      <span
                        class="summary-toggle"
                        class:expanded={expandedEraSummaries.has(era.id)}
                        >&#9658;</span
                      >
                      <span class="summary-title"
                        >Era Summary: Knowledge Loss Assessment</span
                      >
                    </button>
                  ...
                  {#each era.events as event (event.id)}
                    {#if containsTechTag(event)}
                      ...
                      <div class="tech-event" id={event.id}>
                        ...
                        >
                          <span
                            class="tech-toggle"
                            class:expanded={expandedTechEvents.has(event.id)}
                            >&#9658;</span
                          >
                    ...
                    {:else}
                      ...
                      <section
                        class="event"
                        id={event.id}
                        data-primary-type={getPrimaryTag(event)}
                      >
                        <header class="event-header">
                          <span class="event-date">{event.date}</span>
                          <div class="event-tags">
                ...
                {#if era.links && Object.keys(era.links).length > 0}
                  <div class="era-links-section">
                    <button
                      class="era-links-header"
                      onclick={() => toggleEraLinks(era.id)}
                    >
                      <span
                        class="links-toggle"
                        class:expanded={expandedEraLinks.has(era.id)}
                        >&#9658;</span
                      >
                        ...
                        {#each Object.entries(era.links).sort(([a], [b]) => Number(a) - Number(b)) as [num, link]}
                          ...
                          <div class="link-entry">
                            <div class="link-entry-row">
                              <button
                                class="link-label-btn"
                                onclick={() => scrollToEventByEntry(era, num)}
                                >Entry {num}:</button
                              >
                              {#if link.tag}
                                <span class="link-tag" data-type={link.tag}
                                  >{formatTagLabel(link.tag)}</span
                                >
                              {/if}
                              {#if urls[0]}
                                <a
        ...
        <footer class="article-footer">
          ...
          <p class="footer-question">
            What new WHY-frameworks will emerge? Will they be adequate to the
            challenge? Will they help us hold the tension, or collapse us into
...
<style>
  ...
  .stat-card.discovered .stat-label {
    color: #2e7d32;
  }
  .stat-card.recoverable .stat-label {
    color: #ef6c00;
  }
  .stat-card.lost .stat-label {
    color: #c62828;
  }
  ...
  .stat-card.discovered .stat-value {
    color: #1b5e20;
  }
  ...
  .link-entry-row.link-continuation {
    padding-left: calc(
      70px + 0.5rem
    ); /* Align with first URL (Entry X: width + gap) */
  }
  ...
  .link-tag[data-type="tech"] {
    background: #e0e5e8;
    color: #4a5a68;
  }
  .link-tag[data-type="cognitive"] {
    background: #ebe4f0;
    color: #6a5a7a;
  }
  .link-tag[data-type="climate"] {
    background: #e4eaef;
    color: #5a6878;
  }
  .link-tag[data-type="migration"] {
    background: #f2ebe0;
    color: #8a7a60;
  }
  .link-tag[data-type="external"] {
    background: #3a3235;
    color: #c0a8a8;
  }
  .link-tag[data-type="hammerfall"] {
    background: #2d2d2d;
    color: #c08080;
  }
  .link-tag[data-type="collapse"] {
    background: #4a4a4a;
    color: #d0d0d0;
  }
  .link-tag[data-type="expansion"] {
    background: #f5ecd4;
    color: #b8942a;
  }
